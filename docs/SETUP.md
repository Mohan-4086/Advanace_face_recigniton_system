...
Use Python **3.11** with Tkinter, a desktop display and a working webcam. Python's Windows installer normally includes Tkinter. Linux may require your distribution's `python3-tk` package. Dependency installation and non-GUI smoke checks also passed on Python 3.12 in Linux; full desktop behavior on each platform still needs validation.
